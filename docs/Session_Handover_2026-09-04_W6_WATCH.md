...
## 4. Register state — COLLISION, ruling owed (W6-BL1)
...
Count-from-my-own-edit mislabels ×4 in one evening (snippet add_header 4→3; rates 2→3; "a animal" 4→3+1 capital; "Telephone consultation" 3→1 case), all caught by asserts or the instrument (W6-F2). Two recon instrument faults (rev-parse form; gen.py path). `journalctl -u nginx` does not see `nginx -s reload` (W5's reloads, certbot's) — only `systemctl reload` (W6-F3).

...
W-series register entry (ruling A, W6b, 5 Sep 2026: W register lives in `flo4paws-site/docs/`, not `animoit-infra`). Numbering continues from WBL-654 (W3). Census instrument: `grep -c '^### WBL-'`.

## Closures (edit existing rows)

- **WBL-644 → DONE (W4).** PR #3 squash-merged `49fbe88`. `/about/` live on staging; CI read unfiltered before merge. Flo's read-through still owed (content, not a blocker).
- **WBL-645 → DONE (W4).** `location /` carries `Cache-Control: no-cache` (PR #5, `75ab537`). MEASURED: `curl -sI staging/` → `Cache-Control: no-cache`.
- **WBL-646 → DONE-PARTIAL (W4).** Marketing half shipped: `/app/` page, PR #4 `395f711`. Not in primary nav; footer + home links. Help half → WBL-655.
- **WBL-650 → DONE (W4).** `docs/` in repo, PR #6 `d1f4967`. `ia-v2.md` corrected in three cells (spend, cost per conversion, withdrawn PMax recommendation). W2/W3 handovers and the ChatGPT audit not yet in `docs/` (README names the gap).
- **WBL-652 → PARTIAL (W4).** Second login identified: four sessions from Ian's IP, one `flo4paws` pts from 2 Sep. Not a third party. `r2dev.env`, `/tmp` tarballs, and beerscope-in-controls still open (beerscope was used as a control in W4 — 200 before/after reload — but not yet written into any control list).

---

### WBL-655 | OPEN | W4 | RULED | MEDIUM | Help / user guides served by the tenant frontend at `/help/`
...
### WBL-656 | OPEN | W4 | MEASURED | SMALL | Images emit two `Cache-Control` headers
...
### WBL-657 | OPEN | W4 | MEASURED | SMALL | `/app/` copy claims recorded VERBAL; screenshots owed
...
### WBL-658 | OPEN | W4 | INHERITED | SMALL | `docs/` gaps — W2, W3, W4 handovers and the ChatGPT audit
...
### WBL-659 | OPEN | W4 | MEASURED | SMALL | `ia-v2` Ads figure — W1 gives two "corrected" values

...
Append to `animoit-infra/claude/FINDINGS.md`. Numbering continues from F-91 (W4).

---

### F-92 | W5 | MEASURED | `grep -r` on `sites-enabled/` filed a false absence, and a hypothesis was built on it
...
★ F-89's corollary again: before asserting an absence, test the behaviour. And when a grep over `sites-enabled` returns 0, that is the symlink trap until proven otherwise.

---

### F-93 | W5 | MEASURED | six counts labelled DERIVED that were not derived
...
★ F-90's rule was in the brief and still not applied. Stronger form: a number is DERIVED only when a script or command printed it before the prediction was written. "I added these up from the edit I just wrote" is INFERRED and should be labelled so.

---

### F-94 | W5 | MEASURED | an instrument that could not read its input scored as a pass-in-waiting
...
### F-95 | W5 | MEASURED | `gh pr checks --watch` returned before any check existed and the merge went through ungated
...
### F-96 | W5 | MEASURED | a valid-looking measurement ID that Google refuses to serve, five months old
...
### F-97 | W5 | MEASURED | two symptoms, one cause; a second cause was invented
...
### F-98 | W5 | MEASURED | the box's resolver cache was the instrument for a post-DNS control
...
### F-99 | W5 | MEASURED | two instruments measuring the wrong thing, in one block
...
### F-100 | W5 | MEASURED | brief and memory carried three stale facts into the session

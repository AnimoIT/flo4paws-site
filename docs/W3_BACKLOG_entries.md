...
Append to `animoit-infra/claude/BACKLOG.md`. Numbering continues from BL-642 (S216). Census instrument: `grep -c '^### BL-'`.

---

### BL-643 | DONE | W3 | MEASURED | MEDIUM | flo4paws-site had no source control and no generator
...
### BL-644 | OPEN-PR | W3 | MEASURED | MEDIUM | /about/ page, About Flo in nav/footer, real store links, cache-busted assets
...
remedy:   Flo reads `/about/`; Ian checks nav wrap on tablet widths; rule Lincolnshire (BL-649); merge. If wrong: `git checkout main && rsync` restores.
Claim evidence: MEASURED — `/about/` 200 on staging; store hrefs present in served home.

---

### BL-645 | OPEN | W3 | MEASURED | SMALL | HTML pages carry no Cache-Control; ruling on `no-cache`
...
### BL-646 | OPEN | W3 | RULED-PARTIAL | MEDIUM | /app/ page — marketing on the site, help in the tenant frontend

state:    Ian wants an app page that doubles as help. Home already links the stores and sign-in (BL-644). Help content is per-tenant (Lily needs the same with her branding).
...
### BL-647 | OPEN | W3 | MEASURED | SMALL | Newgrange Vets placeholder renders as client-facing text
...
### BL-648 | OPEN | W3 | MEASURED | SMALL | No branch protection on flo4paws-site; merge can outrun CI
...
### BL-649 | OPEN | W1→W3 | RULING | SMALL | Coverage counties — Lincolnshire in or out
...
### BL-650 | OPEN | W3 | INHERITED | SMALL | Move W1 satellite docs into the repo and fix ia-v2 Ads figure
...
### BL-651 | OPEN | W3 | VERBAL | SMALL | Record BL-576 closure, real E2E submission, BL-566 parked — with evidence
...
### BL-652 | OPEN | W3 | MEASURED | SMALL | Hygiene — r2dev.env, /tmp tarballs, second login, beerscope in controls
...
### BL-653 | OPEN | W2→W3 | RULING | SMALL | Nav label "How it works" and site-wide width
...
### BL-654 | OPEN | W1→W3 | INHERITED | MEDIUM | Launch session: GA4 + Consent Mode v2, then cutover

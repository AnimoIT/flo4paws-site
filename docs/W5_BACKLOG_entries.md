...
W-series register entry (ruling A, W6b, 5 Sep 2026: W register lives in `flo4paws-site/docs/`, not `animoit-infra`). Numbering continues from WBL-659 (W4). Census instrument: `grep -c '^### WBL-'`.
...
- **WBL-657 → PARTIAL (W5).** The plan-and-diary claim is gone from `/thank-you/`; still present on `/app/`. Screenshots still owed.
- **WBL-649 → note (W5).** `grep -rl Lincolnshire /var/www/flo4paws-site` = 0 pages at close; the contradiction is gone. The ruling left is whether Flo wants Lincolnshire *added* to the footer/copy.
- **WBL-651 → note.** Cross-domain GA4 + Consent Mode (the last cutover gate item) is done; the register closure itself is still owed here.

---

### WBL-660 | OPEN | W5 | MEASURED | SMALL | staging.flo4paws.co.uk is now indexable
...
### WBL-661 | OPEN | W5 | MEASURED | MEDIUM | DNS zone lives on Wix nameservers; move it before any Wix downgrade
...
### WBL-662 | OPEN | W5 | MEASURED | SMALL | Old GA4 stream and the dead measurement ID
...
### WBL-663 | OPEN | W5 | MEASURED | SMALL | Enhanced-measurement `click` still fires on the app link
...
### WBL-664 | OPEN | W5 | RULED | SMALL | Retire the Wix Ads conversion actions after ≥1 `intake_submitted`
...
### WBL-665 | OPEN | W5 | MEASURED | SMALL | certbot redirect-enhancement conflict; renewal not dry-run
...
### WBL-666 | OPEN | W5 | RULED | SMALL | `--success-url` lives in the publish record only
...
### WBL-667 | OPEN | W5 | INFERRED | SMALL | A3 residue: consent reset path and Consent tab unread
...
### WBL-668 | OPEN | W5 | INHERITED | SMALL | Stage D watch
...
### WBL-673 | OPEN | W5 | MEASURED | SMALL | Search Console ownership: add the HTML-tag meta to `gen.py` HEAD
...
### WBL-669 | OPEN | W5 | MEASURED | SMALL | Wix site stays up; nothing points at it
...
remedy:   Leave up a week; downgrade only after WBL-661. Update Google Business Profile links to `/contact/`.
Claim evidence: MEASURED.

---

### WBL-670 | OPEN | W5 | RULED | SMALL | Intake 502 branch is not counted as a conversion
...
### WBL-671 | OPEN | W5 | MEASURED | SMALL | `flo4paws-dev-api/.env` lacks `GEN_DATABASE_URL`
...
### WBL-672 | OPEN | W5 | INHERITED | SMALL | Cookie policy wording review
...
### WBL-674 | OPEN | W5 | RULED | MEDIUM | Google Ads final URLs still point at Wix-era paths
...
### WBL-675 | OPEN | W5 | MEASURED | SMALL | Reviews: verbatim Trustpilot text or the widget; no review markup to strip
...
### WBL-676 | OPEN | W5 | INHERITED | SMALL | Google Business Profile refresh (audit §5.4/§7.7)
...
### WBL-677 | OPEN | W5 | INHERITED | SMALL | Article cadence on pain-and-behaviour topics (audit §6/§7.8)

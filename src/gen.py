...
"""
import filecmp, html, json, pathlib, shutil, sys, tempfile

SRC = pathlib.Path(__file__).resolve().parent
ROOT = SRC.parent
SITE = "https://www.flo4paws.co.uk"
NAV_ITEMS = [('/', 'Home'), ('/how-it-works/', 'How it works'), ('/guides/', 'Guides'), ('/walks/', 'Walks'), ('/flos-friends/', "Flo's Friends")]
...
def load_pages():
    ...
    for p in sorted((SRC / "pages").glob("*.html")):
        ...
        meta["_file"] = p.name
        pages.append(meta)
    pages.sort(key=lambda m: m["sitemap"]["order"])
    return pages
...
def render_page(m):
    ...
    return (head + '<script type="application/ld+json">' + ld + "</script>"
            + HEAD_TAIL.replace("@@NAV@@", render_nav(m["nav"])) + m["body"] + FOOTER)


def render_sitemap(pages):
    rows = "".join(
        "  <url><loc>%s%s</loc><changefreq>%s</changefreq><priority>%s</priority></url>\n"
        % (SITE, m["path"], m["sitemap"]["changefreq"], m["sitemap"]["priority"]) for m in pages)
